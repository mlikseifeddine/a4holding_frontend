import { Layout, Breadcrumb, Empty, Divider, Row, Col, Tag, Table, Spin } from 'antd';
import {
  CheckCircleOutlined,
  PlayCircleOutlined,
  PauseCircleOutlined,
      ...
      <CustomSider selectedMenu="assessment"/>
        ...
        <CustomHeader title={t("appTitle")} breadcrumb={ <Breadcrumb className="flex-center-row" itemRender={itemRender} routes={routes}/>} ></CustomHeader>
            ...
            <Row className="flex-center-row" gutter={16}>
              ...
              <div className="inner-container">
                  <div style={{marginBottom:"0.5rem",color: "rgba(0, 0, 0, 0.45)",fontSize: "14px"}}>{t("domainmanager")}</div>
                      <div style={{ display: "inline-flex", width: "100%" }}>
                          <TooltipTrunc style={{textAlign: "left", maxWidth: "100%", wordWrap: "break-word", color: "rgba(0, 0, 0, 0.85)", fontSize: "20px"}} text={processinstance && processinstance.process.domainmanager.name}></TooltipTrunc>
                      </div>
                  </div>
              </Col>
            </Row>
          ...
          <div className="data-container">
            ...
            onRow={(record) => {
                return {
                  onClick: () => history.push({pathname:`/assessment/process/compilation`, state: {...location.state, controlinstanceid: record.id}}),
                  };
